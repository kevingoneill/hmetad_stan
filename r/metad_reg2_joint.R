...
data.simulated.stan <- list(N=nrow(d),
                            ...
                            ## for each point, find the corresponding row of X_pred
                            x=d$condition,
                            ##x=as.vector(apply(model.matrix(~ condition, d), 1, function(x)
                            ##    which(apply(model.matrix(~ condition, count(d, condition)), 1,
                            ##                function(xp) all(x == xp))))),
                            stimulus=d$stimulus,
                            response=d$response,
...
## fit stan model
m <- cmdstan_model('../stan/metad_reg2_joint.stan')
