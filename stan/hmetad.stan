...
transformed data {
  ...
  // R[participant, stimulus, response]
  array[P, 2, 2] int R = rep_array(0, P, 2, 2);
  
  // tally up confidence ratings for multinomial likelihood
  // C[participant, response, accuracy, confidence]
  array[P, 2, 2, K] int C;
  for (p in 1:P) {
    C[p] = rep_array(0, 2, 2, K);
  }
  ...
  for (n in 1:N) {
    ...
    R[participant[n], stimulus[n]+1, response[n]+1] += 1;
    C[participant[n], response[n]+1, accuracy+1, confidence[n]] += 1;
  }
}
...
model {
  ...
  if (!prior_only) {
    for (p in 1:P) {
      ...
      for (s in 0:1) {
        target += binomial_lpmf(R[p, s+1, 2] | R[p, s+1, 1] + R[p, s+1, 2],
                                type1_pmf(s, 1, d_prime[p], c[p]));
      }
        
      // model confidence data with multinomial distribution
      for (r in 0:1) {
        for (a in 0:1) {
          target += multinomial_lpmf(C[p, r+1, a+1] |
                                     type2_pmf(r, a, meta_d_prime[p], meta_c[p],
